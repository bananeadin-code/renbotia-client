...
/**
 * Chat de DEMO pública (sin registro): el visitante prueba el bot de ejemplo
 * antes de crear cuenta. Es stateless (reenvía el historial corto al backend).
 * Reutilizable en la landing (sección) y en el widget flotante de soporte.
 */
export function DemoChat({ className = '', heightClass = 'h-[420px]' }) {
  ...
  async function send(text) {
    ...
    try {
      ...
      const data = await demoApi.send(msg, history.slice(-8));
  ...
  return (
    <div className={`flex flex-col overflow-hidden rounded-2xl border border-line bg-surface shadow-pop ${className}`}>
      <ChatHeader botName="Asistente de ejemplo" typing={loading} />
      ...
      {userTurns >= 2 && (
        <div className="flex items-center justify-between gap-3 border-t border-line bg-brand-50/70 px-3 py-2 dark:bg-brand-900/20">
          <span className="text-xs text-muted">Este es un ejemplo. Arma el tuyo con tu info.</span>
          <Link to="/registro">
            <Button size="sm">Crear mi bot gratis</Button>
          </Link>
        </div>
      )}
